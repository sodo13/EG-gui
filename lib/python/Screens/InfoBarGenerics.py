...
class SecondInfoBar(Screen):
    ...
    def __init__(self, session):
        ...
        if config.usage.second_infobar_style.value == 'thirdinfobar':
            self.skinName = ['ThirdInfoBar', 'SecondInfoBar']
        elif config.usage.second_infobar_style.value == 'secondinfobar':
...
class InfoBarPiP():
    ...
    def showPiP(self):
        if self.session.pipshown:
            ...
            if info and int(xres) <= 720 or getMachineBuild() not in ('blackbox7405', 'inihdx'):
